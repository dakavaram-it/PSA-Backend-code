...
def revert_committee_stage(
    proposalId: int = Path(...),
    actionBy: int = Query(0, ge=0),
    actionByName: str = Query(""),
    service: CommitteeService = Depends(get_service),
):
    """Move the committee proposal one workflow stage back (detail "Back" button)."""
    try:
        return ApiResponse(data=service.revert_to_previous_stage(proposalId, actionBy, actionByName))
    except Exception as exc:
        handle_error("committee_revert_stage", exc)
...
def committee_save_reviews(req: CommitteeReviewSaveRequest, proposalId: int = Path(...), service: CommitteeService = Depends(get_service)):
    try:
        return ApiResponse(data=service.save_reviews_only(
            proposalId, req.reviews, req.actionBy, req.actionByName or "",
        ))
    except Exception as exc:
        handle_error("committee_save_reviews", exc)
