import logging
from app.repositories.committee_repository import CommitteeRepository
from app.utils.cadre_photo import filter_valid_mids
...
class CommitteeService:
    ...
    def revert_to_previous_stage(self, proposal_id: int, action_by: int = 0, action_by_name: str = ""):
        logger.info("committee revert stage proposal_id=%s user=%s", proposal_id, action_by)
        return self.repo.revert_to_previous_stage(proposal_id, action_by, action_by_name)
    ...
    def save_reviews_only(self, proposal_id: int, reviews, action_by: int, action_by_name: str | None):
        logger.info("committee save reviews only proposal_id=%s count=%s", proposal_id, len(reviews or []))
        return self.repo.save_reviews_only(proposal_id, reviews, action_by, action_by_name)
